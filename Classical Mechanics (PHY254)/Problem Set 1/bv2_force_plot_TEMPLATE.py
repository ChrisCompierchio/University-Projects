...
#velocity
v = vT*(np.tanh((g*t+v0)/(vT))) # insert your expression here

#position
x = ((vT**2)/g)*np.log(np.cosh(((g*t + v0)*t)/vT)) # insert your expression here

#acceleration
a = vT - ((np.tanh((g*t + v0)/vT))**2)*g # insert your expression here
...
for i in range(1,npts):    # range function omits last index at npts, starts at 1 not zero
    vn[i] = vT*(np.tanh((g*t[i]+vn[0])/(vT))) # insert your expression here

xn[0] = x0  # set initial condition as first member of array

for i in range(1,npts): 
    xn[i] = ((vT**2)/g)*np.log(np.cosh(((g*t[i] + vn[0])*t[i])/vT)) # insert your expression here

for i in range(0,npts):     # go over the whole range 0 .. npts-1
    an[i] = vT - ((np.tanh((g*t[i] + vn[0])/vT))**2)*g # insert your expression here
